...
contract DawnDeposit is IDawnDeposit, DawnTokenPETH, DawnBase {
    ...
    function handleOracleReport(uint256 beaconValidators, uint256 beaconBalance, uint256 availableRewards) external {
        ...
        require(
            availableRewards <= getContractAddress(REWARDS_VAULT_CONTRACT_NAME).balance,
            "RewardsVault insufficient balance"
        );
        require(
            beaconBalance.add(availableRewards) >=
                getUint(BEACON_BALANCE_KEY).add(
                    beaconValidators.sub(getUint(BEACON_VALIDATORS_KEY)).mul(DEPOSIT_VALUE_PER_VALIDATOR)
                ),
            "unprofitable"
        );
        ...
        uint256 rewardsPEth = preTotalPEth.mul(rewards).mul(getUint(FEE_KEY)).div(
            preTotalEther.mul(FEE_BASIC).add(rewards.mul(FEE_BASIC.sub(getUint(FEE_KEY))))
        );
        // distributeRewards
    ...
    function getTotalPooledEther() public view returns (uint256) {
        return
            getUint(BUFFERED_ETHER_KEY).add(getUint(BEACON_BALANCE_KEY)).add(
                getUint(DEPOSITED_VALIDATORS_KEY).sub(getUint(BEACON_VALIDATORS_KEY)).mul(DEPOSIT_VALUE_PER_VALIDATOR)
            );
    }
